...
// Mostrar información de todos los proyectos
...
GROUP BY PROYECTO, area_funcional";
...
if ($result_aprobado && $result_aprobado->num_rows > 0) {
    while($row_aprobado = $result_aprobado->fetch_assoc()) {
        $proy = isset($row_aprobado['PROYECTO']) ? $row_aprobado['PROYECTO'] : '';
        $area = isset($row_aprobado['area_funcional']) ? $row_aprobado['area_funcional'] : '';
        if (!isset($costos_imputados_aprobados[$proy])) $costos_imputados_aprobados[$proy] = [];
        $costos_imputados_aprobados[$proy][$area] = $row_aprobado['total_imputado'];
    }
}


// Traer porcentajes de avance físico ejecutado y programado por área funcional para el proyecto desde la tabla correcta
$sql_avance = "SELECT PROYECTO, AREA_FUNCIONAL, PORCENTAJE_AVANCE_FISICO_EJECUTADO, PORCENTAJE_AVANCE_FISICO_PROGRAMADO FROM avance_fisico_ejecutado_programado";
$avance_porcentajes = [];
...
if ($result_avance && $result_avance->num_rows > 0) {
    while($row = $result_avance->fetch_assoc()) {
        ...
        if (!isset($avance_porcentajes[$proy])) $avance_porcentajes[$proy] = [];
        $avance_porcentajes[$proy][$area] = [
...
// Calcular el total por área funcional y proyecto (sin versiones)
$totales_area = [];
foreach ($filas as $f) {
    $proy = $f['PROYECTO'];
    ...
    if (!isset($totales_area[$proy])) $totales_area[$proy] = [];
    if (!isset($totales_area[$proy][$area])) $totales_area[$proy][$area] = 0;
    $totales_area[$proy][$area] += $suma_horas * (is_numeric($tarifa) ? (float)$tarifa : 0);
}
// Unir todas las áreas posibles por proyecto
$areas = [];
foreach ($totales_area as $proy => $areas_arr) {
    foreach (array_keys($areas_arr) as $a) {
        $areas[$proy][$a] = true;
    }
}
// ...existing code...
?>
    ...
    <div class="mb-4" style="width: 100%; max-width: 100%; padding: 0; margin: 0;">
        <ul class="nav nav-tabs" id="multiMenuTabs" role="tablist" style="width: 100%;">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="menu2-tab" data-tab="menu2" type="button" role="tab" aria-controls="menu2" aria-selected="false">APROBACIÓN</button>
            </li>
        </ul>
        <div class="border border-top-0 rounded-bottom p-3" style="width: 100%; max-width: 100%; background: #fff;">
            ...
            <div id="menu2" class="tab-pane-content d-none" style="width: 100%;">
                <?php
                            ...
                            $vista_sql = "SELECT PROYECTO, nombre_proyecto, area_funcional, total_imputado, total_aprobado, porcentaje_aprobacion, porcentaje_aprobacion_director FROM aprobacion_area_funcional_proyecto";
                            ...
                            if ($vista_rs === false) {
                                ...
                                while ($r = $vista_rs->fetch_assoc()) {
                                    ...
                                    echo '<td>' . $fmt_bac . '</td>';
                                    // Costo Actual (AC) por área funcional (desde costo_valorizado)
                                    $ac_area = (isset($costos_actuales_areas[$proyecto]) && isset($costos_actuales_areas[$proyecto][$af])) ? (float)$costos_actuales_areas[$proyecto][$af] : 0;
    ...
    <div class="mb-4" id="menu1_source_block" style="width: 100%; max-width: 100%;">
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                        ...
                        <table class="table table-bordered table-hover align-middle shadow-sm" id="tablaResumenVersion">
                            ...
                            <tbody>
                                ...
                                foreach ($filas as $f) {
                                    ...
                                    $resumenes[$key]['HORAS'] += $suma_horas;
                                    $resumenes[$key]['COSTO'] += $suma_horas * (is_numeric($tarifa) ? (float)$tarifa : 0);
                                }
                    ...
                    <h5 class="card-title mt-4">DETALLE PRESUPUESTO CARGADO EN HH</h5>
                    <div class="table-responsive" id="tablasDetalle">
                        ...
                        foreach ($cols_mostrar_final as $colname) {
                            $thead .= "<th style='background-color:#4C8AA3; color:white; font-weight:bold; white-space:nowrap; overflow:auto; word-break:break-all;'>" . htmlspecialchars($colname, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</th>";
                        }
                        $thead .= "</tr>";
                        $tbody = "";
                        $total_horas = 0;
                    ...
                    <div class="table-responsive mt-5">
                        ...
                        foreach ($cols_mostrar_final_costo as $colname) {
                            $thead_costo .= "<th style='background-color:#17823d; color:white; font-weight:bold; white-space:nowrap; overflow:auto; word-break:break-all;'>" . htmlspecialchars($colname, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "</th>";
                        }
                        $thead_costo .= "</tr>";
                        $tbody_costo = "";
                        $total_plata_costo = 0;
                                ...
                                if (celda) {
                                    if (esMes) {
                                        ...
                                        for (var i = 0; i < filas.length - 1; i++) {
                                            ...
                                            hayVisible = true;
                                            var celdas = filas[i].cells;
                                            var val = celdas[j].textContent.replace(/[^\d]/g, '');
                                            if (val && !isNaN(val)) suma += parseInt(val);
                                        }
                                        if (tipo === 'costo') {
                                            celda.textContent = '$ ' + (hayVisible ? suma.toLocaleString('es-CO', {maximumFractionDigits:0}) : '0');
                                        } else if (tipo === 'horas') {
                                            celda.textContent = hayVisible ? parseInt(suma) : '0';
                                        } else {
                                            celda.textContent = '';
                                        }
                                    } else {
